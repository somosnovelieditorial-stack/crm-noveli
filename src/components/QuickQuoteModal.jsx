...
• Los valores indicados son referenciales hasta la aceptación formal del cliente y confirmación de pago.`,
  ...
  const getTotals = () => {
    const totals = calculateProposalTotals(formHeader, formItems);
    console.log("proposal totals usados en preview", totals);
    return totals;
  };
                          ...
                          <tbody>
                            {getPreviewRows(formItems, totals, formHeader).map(row => (
                              <tr key={row.id} className="border-b border-slate-50 dark:border-slate-855">
                                ...
                                <td className="p-2 text-right font-mono font-bold text-slate-700 dark:text-slate-355">{formatCurrency(row.total, formHeader.currency)}</td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
                          ...
                          {includedItems.length === 0 ? (
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-[10px] text-slate-600 dark:text-slate-350">
                              {includedItems.map((item, idx) => {
                                ...
                                return (
                                  <div key={idx} className="text-[11px] py-1 border-b border-slate-50 dark:border-slate-850 pb-1 flex flex-col gap-0.5">
                                    <div className="flex gap-2">
